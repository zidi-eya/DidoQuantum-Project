...
def read_researcher_profile(profile_id: int, db: Session = Depends(get_db)):
    profile = researcher_profile_service.get_profile(db, profile_id)
    if not profile:
...
def read_researcher_profiles(skip: int = 0, limit: int = 10, db: Session = Depends(get_db)):
    profiles = researcher_profile_service.get_profiles(db)
...
def delete_researcher_profile(profile_id: int, db: Session = Depends(get_db)):
    researcher_profile_service.delete_profile(db, profile_id)
